...
public class HorizontalZigZagSlicer extends Slicer {
    ...
    public HorizontalZigZagSlicer(int i, int i2, int i3, int i4) {
        super(i, i2, i3, i4);
    ...
    public void createPath(float f) {
        ...
        float f2 = (((float) this.screenWidth) * 1.0f) / 50.0f;
        float f3 = 0.0f;
        while (f3 <= ((float) this.screenWidth)) {
            float f4 = f3 + f2;
            this.path.lineTo(f4, 2.0f * f);
            f3 = f4 + f2;
            this.path.lineTo(f3, (float) this.increment);
        }
        this.path.lineTo((float) this.screenWidth, 0.0f);
        this.path.moveTo(0.0f, 0.0f);
        float f5 = 0.0f;
        while (f5 <= ((float) this.screenWidth)) {
            float f6 = f5 + f2;
