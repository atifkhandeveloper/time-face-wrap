...
public class VerticalZigZagSlicer extends Slicer {
    ...
    public VerticalZigZagSlicer(int i, int i2, int i3, int i4) {
        super(i, i2, i3, i4);
    ...
    public void drawSlice(Bitmap bitmap, Bitmap bitmap2, IImageFilter iImageFilter) {
        ...
        if (bitmap3 == null) {
            this.curveBitmap = Bitmap.createBitmap(this.pathBounds.width(), this.pathBounds.height(), Bitmap.Config.ARGB_8888);
        } else {
    ...
    public boolean isScanDone() {
        return this.scrollingSliceRect.left >= this.screenWidth;
    }
    ...
    private Point getScanPosition(Point point) {
        ...
        this.matrix.reset();
        this.matrix.setTranslate((float) this.increment, 0.0f);
        this.scannerPath.transform(this.matrix);
        Rect rect = this.pathBounds;
        rect.set(rect.left + this.increment, this.pathBounds.top, this.pathBounds.right + this.increment, this.pathBounds.bottom);
        this.scrollingSliceRect.set(this.scrollingSliceRect.left + this.increment, this.scrollingSliceRect.top, this.scrollingSliceRect.right + this.increment, this.scrollingSliceRect.bottom);
        return point;
    }
    ...
    public void createPath(float f) {
        ...
        this.path.lineTo((float) this.increment, 0.0f);
        float f2 = (((float) this.screenHeight) * 1.0f) / 50.0f;
        float f3 = 0.0f;
        while (f3 <= ((float) this.screenHeight)) {
            float f4 = f3 + f2;
            this.path.lineTo(2.0f * f, f4);
            f3 = f4 + f2;
            this.path.lineTo((float) this.increment, f3);
        }
        this.path.lineTo(0.0f, (float) this.screenHeight);
        this.path.moveTo(0.0f, 0.0f);
        float f5 = 0.0f;
        while (f5 <= ((float) this.screenHeight)) {
            float f6 = f5 + f2;
        ...
        this.sliceBitmap = Bitmap.createBitmap(this.pathBounds.width(), this.screenHeight, Bitmap.Config.ARGB_8888);
        this.scrollingSliceRect = new Rect(this.pathBounds.left - this.pathBounds.right, this.pathBounds.top, this.pathBounds.right - this.pathBounds.right, this.pathBounds.bottom);
        this.matrix.setTranslate((float) (-this.pathBounds.right), 0.0f);
